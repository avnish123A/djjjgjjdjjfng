...
const categoryOptions = ['fashion', 'electronics', 'beauty', 'home-living', 'sports', 'grocery'];
...
const AdminProductForm: React.FC = () => {
  ...
  const [form, setForm] = useState({
    ...
    stock: '50',
    isActive: existing?.inStock ?? true,
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // TODO: Replace with API call to your Express backend
    // const formData = new FormData();
    // formData.append('title', form.title);
    // ... etc
    // await apiUpload(apiConfig.endpoints.addProduct, formData);
        ...
        <div className="bg-card border border-border rounded-xl p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="space-y-2">
              ...
              <Input id="price" type="number" value={form.price} onChange={(e) => updateField('price', e.target.value)} placeholder="0" min="0" required />
            </div>
            <div className="space-y-2">
              <Label htmlFor="originalPrice">Original Price (₹)</Label>
              <Input id="originalPrice" type="number" value={form.originalPrice} onChange={(e) => updateField('originalPrice', e.target.value)} placeholder="0" min="0" />
            </div>
            <div className="space-y-2">
        ...
        <div className="bg-card border border-border rounded-xl p-6 space-y-4">
          ...
          <div className="flex flex-wrap gap-3">
            {form.images.map((file, i) => (
              <div key={i} className="relative w-24 h-24 rounded-lg overflow-hidden border border-border bg-secondary">
                <img src={URL.createObjectURL(file)} alt="" className="w-full h-full object-cover" />
                <button
                  type="button"
                  onClick={() => removeImage(i)}
                  className="absolute top-1 right-1 p-0.5 bg-foreground/60 rounded-full text-background"
                >
                  <X className="h-3 w-3" />
                </button>
              </div>
              ...
              {colorOptions.map(color => (
                <button
                  ...
                  className={`flex items-center gap-2 px-3 py-2 rounded-lg border text-sm transition-colors ${
                    form.selectedColors.includes(color.value)
